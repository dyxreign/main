...
local Themes = {
    ...
    Light = {
        ...
        Card = Color3.fromRGB(220, 220, 225),
    ...
    Amethyst = {
        ...
        Accent = Color3.fromRGB(153, 50, 204),
        Hover = Color3.fromRGB(45, 35, 65)
    },
    Bloom = {
        Bg = Color3.fromRGB(30, 15, 20),
        Card = Color3.fromRGB(45, 25, 30),
        Stroke = Color3.fromRGB(80, 40, 50),
        Text = Color3.fromRGB(255, 230, 235),
        SubText = Color3.fromRGB(200, 170, 180),
        Accent = Color3.fromRGB(255, 105, 180),
        Hover = Color3.fromRGB(65, 35, 45)
    },
    ["Dark Blue"] = {
        Bg = Color3.fromRGB(10, 15, 25),
        Card = Color3.fromRGB(15, 22, 35),
        Stroke = Color3.fromRGB(30, 45, 70),
        Text = Color3.fromRGB(230, 240, 255),
        SubText = Color3.fromRGB(160, 180, 210),
        Accent = Color3.fromRGB(65, 105, 225),
        Hover = Color3.fromRGB(25, 35, 55)
    },
    Green = {
        Bg = Color3.fromRGB(15, 25, 15),
        Card = Color3.fromRGB(22, 35, 22),
        Stroke = Color3.fromRGB(45, 70, 45),
        Text = Color3.fromRGB(230, 255, 230),
        SubText = Color3.fromRGB(170, 210, 170),
        Accent = Color3.fromRGB(46, 139, 87),
        Hover = Color3.fromRGB(35, 55, 35)
    },
    Ocean = {
        Bg = Color3.fromRGB(10, 25, 30),
        Card = Color3.fromRGB(15, 35, 45),
        Stroke = Color3.fromRGB(30, 70, 90),
        Text = Color3.fromRGB(220, 245, 255),
        SubText = Color3.fromRGB(150, 190, 210),
        Accent = Color3.fromRGB(0, 191, 255),
        Hover = Color3.fromRGB(25, 50, 65)
    }
}
...
function LuannyUi:Notify(options)
    ...
    local hasIcon = type(iconName) == "string" and iconName ~= ""
    local textRightOffset = 12

    if hasIcon and Icons[iconName] then
        textRightOffset = 38
...
function LuannyUi:CreateWindow(config)
    ...
    Toolbar.AnchorPoint = (WindowConfig.Position == "Top" and Vector2.new(0.5, 0)) or
                          (WindowConfig.Position == "Left" and Vector2.new(0, 0.5)) or
                          (WindowConfig.Position == "Right" and Vector2.new(1, 0.5)) or
                          Vector2.new(0.5, 1)
    ...
    BtnToggleBar.BorderSizePixel = 0
...
function LuannyUi:Tab(options)
    ...
    local TabData = {Container = container, ItemCount = 0, LastLabelRow = nil}
...
function TabClass:Section(options)
    ...
    return {
        SetTitle = function(selfArg, t)
            local val = (type(selfArg) == "table" and selfArg.SetTitle) and t or selfArg
            lbl.Text = type(val) == "table" and "" or tostring(val or "")
        end,
        Destroy = function()
...
function TabClass:AddLabel(options)
    ...
    local titleText = options.Title and tostring(options.Title) or ""
    local contentText = options.Content and tostring(options.Content) or "Label"
    ...
    local rowFrame
    if inline and self.LastLabelRow then
        rowFrame = self.LastLabelRow
        for _, child in ipairs(rowFrame:GetChildren()) do
            if child:IsA("Frame") then
                child.Size = UDim2.new(0.5, -4, 1, 0)
            end
        end
    else
        rowFrame = Instance.new("Frame", self.Container)
        rowFrame.Size = UDim2.new(0, 340, 0, 50)
        rowFrame.BackgroundTransparency = 1
        rowFrame.LayoutOrder = self.ItemCount
        rowFrame.BorderSizePixel = 0
        self.LastLabelRow = rowFrame
    ...
    card.Size = inline and UDim2.new(0.5, -4, 1, 0) or UDim2.new(1, 0, 1, 0)
    ...
    lblContent.BorderSizePixel = 0
    RegisterText(lblContent)

    return {
        Set = function(selfArg, newContent, newColor)
            local isColon = type(selfArg) == "table" and selfArg.Set
            local txt = isColon and newContent or selfArg
            local col = isColon and newColor or newContent

            if txt ~= nil then
                lblContent.Text = type(txt) == "table" and "" or tostring(txt)
            end
            if col ~= nil and typeof(col) == "Color3" then
                lblContent.TextColor3 = col
            end
        end,
        SetTitle = function(selfArg, newTitle)
            local val = (type(selfArg) == "table" and selfArg.SetTitle) and newTitle or selfArg
            lblTitle.Text = type(val) == "table" and "" or tostring(val or "")
        end,
        Destroy = function()
            card:Destroy()
            if #rowFrame:GetChildren() <= 1 then
                rowFrame:Destroy()
                if self.LastLabelRow == rowFrame then
                    self.LastLabelRow = nil
                end
            else
                for _, child in ipairs(rowFrame:GetChildren()) do
...
function TabClass:Button(options)
    ...
    return {
        ...
        SetDesc = function(selfArg, d)
            local val = (type(selfArg) == "table" and selfArg.SetDesc) and d or selfArg
            lblDesc.Text = type(val) == "table" and "" or tostring(val or "")
        end,
        Lock = function()
...
function TabClass:Toggle(options)
    ...
    local hasIcon = type(options.Icon) == "string" and options.Icon ~= ""
    local textOffset = hasIcon and 35 or 10
    ...
    return {
        ...
        SetValue = function(selfArg, v)
            local val = (type(selfArg) == "table" and selfArg.SetValue) and v or selfArg
            state = type(val) == "boolean" and val or false
            updateVisual()
        end,
        Lock = function()
            lockOverlay.Visible = true
...
function TabClass:Input(options)
    ...
    local hasIcon = type(options.InputIcon) == "string" and options.InputIcon ~= ""
    local iconOffset = hasIcon and 35 or 10
    ...
    return {
        ...
        SetPlaceholder = function(selfArg, p)
            local val = (type(selfArg) == "table" and selfArg.SetPlaceholder) and p or selfArg
            box.PlaceholderText = type(val) == "table" and "" or tostring(val or "")
        end,
        SetValue = function(selfArg, v)
            local val = (type(selfArg) == "table" and selfArg.SetValue) and v or selfArg
            box.Text = type(val) == "table" and "" or tostring(val or "")
        end,
        Lock = function()
            lockOverlay.Visible = true
            TweenService:Create(lockOverlay, TweenInfo.new(0.2), {BackgroundTransparency = 0.5}):Play()
...
function TabClass:Dropdown(options)
    ...
    for _, optName in ipairs(optionList) do
        ...
        optBtn.MouseButton1Click:Connect(function()
            ...
            if options.Callback then
                task.spawn(options.Callback, currentValue)
            end
        end)
    end
    ...
    return {
        ...
        Set = function(selfArg, v)
            local val = (type(selfArg) == "table" and selfArg.Set) and v or selfArg
            currentValue = val
            selectBtn.Text = " " .. tostring(currentValue)
        end,
        Get = function()
...
function TabClass:Slider(options)
    ...
    return {
        ...
        SetMin = function(selfArg, min)
            local val = (type(selfArg) == "table" and selfArg.SetMin) and min or selfArg
            if type(val) == "number" then
                minVal = val
                updateValue(currentVal, true)
            end
        end,
        SetMax = function(selfArg, max)
            local val = (type(selfArg) == "table" and selfArg.SetMax) and max or selfArg
            if type(val) == "number" then
                maxVal = val
                updateValue(currentVal, true)
            end
