...
function LuannyUi:CreateWindow(config) 
    ...
    if WindowConfig.Position == "Top" then
        ...
        barHidPos = UDim2.new(0.5, 0, 0, -45)
    ...
    elseif WindowConfig.Position == "Left" then
        ...
        barHidPos = UDim2.new(0, -45, 0.5, 0)
    ...
    elseif WindowConfig.Position == "Right" then
        ...
        barHidPos = UDim2.new(1, 45, 0.5, 0)
        ...
        barHidPos = UDim2.new(0.5, 0, 1, 45)
    ...
    local toolbarStroke = Instance.new("UIStroke", Toolbar)
    toolbarStroke.Color = strokeColor 
    ...
    if useIntro then
        ...
        task.spawn(function()
            local loadingTitleText = config.LoadingTitle or "Luanny Interface Suite"
            local loadingSubtitleText = config.LoadingSubtitle or "by nyx"
            local loadingIconStr = config.LoadingIcon or "rbxassetid://139490162033546"

            local introGui = Instance.new("ScreenGui", CoreGui)
            introGui.IgnoreGuiInset = true
            
            local introContainer = Instance.new("Frame", introGui)
            introContainer.Size = UDim2.new(1, 0, 1, 0)
            introContainer.BackgroundTransparency = 1
            
            local logo = Instance.new("ImageLabel", introContainer)
            logo.Size = UDim2.new(0, 36, 0, 36)
            logo.AnchorPoint = Vector2.new(0.5, 0.5)
            logo.Position = UDim2.new(0.5, 0, 0.5, 0)
            logo.BackgroundTransparency = 1
            if Icons[loadingIconStr] then
                logo.Image = Icons[loadingIconStr]
            else
                logo.Image = loadingIconStr
            end
            logo.ImageTransparency = 1
            
            local textContainer = Instance.new("Frame", introContainer)
            textContainer.AnchorPoint = Vector2.new(0, 0.5)
            textContainer.Position = UDim2.new(0.5, 10, 0.5, 0)
            ...
            title.FontFace = FontTitle
            title.TextSize = 20
            title.TextXAlignment = Enum.TextXAlignment.Left
            title.TextTransparency = 1
            
            local sub = Instance.new("TextLabel", textContainer)
            sub.Text = loadingSubtitleText
            ...
            textContainer.Size = UDim2.new(0, math.max(title.TextBounds.X, sub.TextBounds.X) + 10, 0, 36)
            
            TweenService:Create(logo, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageTransparency = 0}):Play()
            task.wait(0.6)
            
            local totalWidth = logo.AbsoluteSize.X + textContainer.AbsoluteSize.X + 20
            TweenService:Create(logo, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position = UDim2.new(0.5, -(totalWidth / 2) + 18, 0.5, 0)}):Play()
            textContainer.Position = UDim2.new(0.5, -(totalWidth / 2) + 48, 0.5, 0)
            
            task.wait(0.3)
            TweenService:Create(title, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextTransparency = 0}):Play()
            TweenService:Create(sub, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextTransparency = 0}):Play()
            
            task.wait(2)
            TweenService:Create(logo, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageTransparency = 1}):Play()
            TweenService:Create(title, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextTransparency = 1}):Play()
            TweenService:Create(sub, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextTransparency = 1}):Play()
            ...
            if WindowConfig.ShowWindow then
                ...
                IsExpanded = true
                if isVertical then
                    ExpandBtn.Image = Icons["chevron-up"]
                else
                    ExpandBtn.Image = Icons["chevron-left"]
                end
                UpdateToolbarWidth()
                ...
                IsExpanded = false
                if isVertical then
                    ExpandBtn.Image = Icons["chevron-down"]
                else
                    ExpandBtn.Image = Icons["chevron-right"]
                end
                UpdateToolbarWidth()
            end
        end)
...
function LuannyUi:Tab(options) 
    ...
    local tabWindowStroke = Instance.new("UIStroke", frame)
    tabWindowStroke.Color = winStrokeColor
...
function TabClass:AddLabel(options) 
    ...
    if inline and self.LastLabelWrapper then
        ...
        rowFrame.Size = UDim2.new(1, -15, 1, 0)
        ...
        rowLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
            ...
            indicator.Visible = rowLayout.AbsoluteContentSize.X > rowFrame.AbsoluteSize.X
        end)
...
function TabClass:ColorPicker(options)
    ...
    local function makeInput(txt, isHex)
        ...
        local inputStroke = Instance.new("UIStroke", frame)
